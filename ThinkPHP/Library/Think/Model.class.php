...
class Model {
    ...
    public function csave($data='',$options=array()) {
        if (!$data) {
            $data = $this->data;
        }
        if (!$this->create($data, 2)) {
            header("Content-type:text/html;charset=utf-8");
            echo '<script>alert("'.$this->getError().'");history.go(-1);</script>';
            exit;
        }
        if ($this->save($this->data, $options) === false) {
            header("Content-type:text/html;charset=utf-8");
            echo '<script>alert("'.$this->getError().'");history.go(-1);</script>';
            exit;
        }
    ...
    public function cadd($data='',$options=array(),$replace=false) {
        if (!$data) {
            $data = $this->data;
        }
        if (!$this->create($data)) {
        ...
        $newId = $this->add($this->data, $options, $replace);
        if ($newId === false) {
            header("Content-type:text/html;charset=utf-8");
            echo '<script>alert("'.$this->getError().'");history.go(-1);</script>';
            exit;
